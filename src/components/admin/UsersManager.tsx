...
export default function UsersManager({ currentUserId }: { currentUserId: string }) {
  ...
  const [pendingAction, setPendingAction] = useState<'role' | 'status' | null>(null);
  ...
  const update = async (change: { role?: UserRole; accountStatus?: 'active' | 'suspended' }) => {
    ...
    setPendingAction(change.role ? 'role' : 'status');
    try {
  ...
  return <div className={`admin-task-layout users-task-layout${selected ? ' is-inspecting' : ''}`}>
    ...
    <section className="admin-panel admin-task-detail" tabIndex={-1} ref={detailRef} aria-label={t.action}>
      ...
      {selected ? <div className="admin-form-stack animate-rise">
        ...
        <dl className="admin-facts"><div><dt>{t.accountId}</dt><dd className="mono-id">{selected.userId}</dd></div><div><dt>{t.email}</dt><dd>{selected.maskedEmail ?? t.anonymized}{!selected.emailVerified && t.unverified}</dd></div>
        ...
        {!canGovern ? <Notice kind="info">{selected.userId === currentUserId ? t.selfProtected : t.anonymizedProtected}</Notice> : <>
          ...
          <ReasonPanel reason={reason} onReasonChange={setReason} disabled={command.locked}
            fields={<div className="form-row admin-users-fields">
              <TextField label={t.confirmation} mono value={confirmation} disabled={command.locked} autoComplete="off" spellCheck={false} placeholder={selected.userId} onChange={(event) => setConfirmation(event.target.value)} />
              ...
              {role === 'admin' && selected.role !== 'admin' && <Notice kind="warning" className="form-row-wide">{t.adminWarning}</Notice>}
            </div>}>
            <Button variant={selected.accountStatus === 'suspended' ? 'secondary' : 'danger'} icon={selected.accountStatus === 'suspended' ? 'refresh' : 'lock'} disabled={!ready} loading={pendingAction === 'status'} onClick={() => void update({ accountStatus: selected.accountStatus === 'suspended' ? 'active' : 'suspended' })}>{selected.accountStatus === 'suspended' ? t.restore : t.suspend}</Button>
            <Button variant="primary" icon="users" disabled={!ready || role === selected.role} loading={pendingAction === 'role'} onClick={() => void update({ role })}>{t.changeRole}</Button>
          </ReasonPanel>
        </>}
        {notice}
      </div> : <div className="admin-form-stack">{notice}<AdminEmpty icon="users" title={c.select} /></div>}
    </section>
