...
| 14 文案与决策 | `designSystem.test.ts`（组件内零硬编码中文）通过；ADR-0023/0024/0025 已写；CONTEXT 新增 D57–D63 与 R14 事实段；CHANGELOG 增「未发布」小节；`.env.example` 补 R14 新增变量 |
...
- 既有 `12-community-governance` 的迁移号断言随 0016 更新。
...
4. **迁移 0016 未对真实 PostgreSQL 执行**：SQL 由 PGlite 测试客户端每次从零重放验证；仍未跑 `node db/migrate.cjs`（未访问任何远程库）。
