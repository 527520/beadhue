 ...
 *
 * 场景：
 * - 工作台「公开到豆社」→ 投稿页：交接已解码原图用于预览，上传优先复用私人资产；
 * - 豆社「用这张制作」成功 → 工作台：引用者取回作者原图，绑定到新设计以继续裁剪 / 调参。
 *
 * 此库不承担私人原图的长期保存（由 originals 客户端缓存和队列负责）。
...
const DB_NAME = 'beadhue-pending-originals';
const DB_VERSION = 2;
...
function open(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains('_brand_migrations')) db.createObjectStore('_brand_migrations');
    ...
    request.onsuccess = () => { clearTimeout(timer); void migrateLegacyDatabase(request.result).then(() => resolve(request.result), reject); };
