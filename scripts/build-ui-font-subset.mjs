...
const staging = mkdtempSync(join(tmpdir(), 'beadhue-renamed-fonts-'));
const pythonIndex = process.argv.indexOf('--python');
const python = pythonIndex < 0 ? 'python3' : process.argv[pythonIndex + 1];
const sources = [
  { file: 'NotoSansSC-VF.ttf', sha256: 'd68bafcb48a2707749396aa12bbbd833cb70401f3a9a689fd2902c7e0d295964', name: 'BeadHue Text', stem: 'text', weight: '100 900', license: 'NotoSans-LICENSE.txt' },
  { file: 'ChillRoundF.ttf', sha256: '7dae804b344f7bc1a1c8426b9515e9b54d7baac3e123263d0bae94d0a305a732', name: 'BeadHue Round', stem: 'round', weight: '400', license: 'ChillRound-LICENSE.txt' },
];
...
for (const source of sources) {
  ...
  const displayCopy = new Set([...'豆色绘把喜欢，一颗颗拼出来。下一张，想拼什么？下一份喜欢，从这里开始。创作一张图纸留住你想拼的部分值得一颗一颗拼起来我的设计色板从第一颗豆开始你的设计已保存账号以豆为点以色成画'].map((char) => char.codePointAt(0)));
