...
```text
选择图片 → 新建图纸（取景 / 色板 / 规格）→ 编辑修补 / 调参 → 跟拼 / 导出
                                               └→ 保存设计 / 只读分享 / 公开到豆社
```

[界面预览](#界面预览) · [功能亮点](#功能亮点) · [色板与制作规格](#色板与制作规格) · [页面与交互](#页面与交互) · [数据与隐私](#数据与隐私) · [快速开始](#快速开始) · [生产部署](#生产部署) · [更新日志](CHANGELOG.md)

## 界面预览

### 发现

首页即作品发现：按类目浏览、搜索标题 / 标签 / 作者，筛选与排序；作品卡片是服务端渲染的豆粒缩略图，可直接点喜欢。

![豆色绘发现页：类目条、新手提示与作品网格](docs/screenshots/discover.jpg)

### 作品详情

中性舞台上的图纸查看器（豆粒 / 方格、缩放平移、网格与色号随缩放出现），旁边是制作卡：尺寸、颜色数、总颗数、底板数与色号清单；登录后可「用这张制作」创建自己的私人副本。

![作品详情：图纸查看器与制作卡](docs/screenshots/detail.jpg)

### 编辑器

选图后在「新建图纸」弹窗里取景、选色板与规格并生成首版，进入编辑器继续精修：左侧工具（手形、画笔、橡皮、油漆桶、吸管、替换颜色），右侧颜色 / 调整 / 信息面板，顶栏切换编辑与跟拼、分享与导出。

![编辑器：工具栏、图纸画布与颜色面板](docs/screenshots/editor.jpg)

> 截图来自本地 E2E 种子库与样例作品（`node docs/screenshots/capture.mjs` 生成），不代表线上数据。
>
...
| 模块 | 当前能力 |
|---|---|
| 本地制图 | 图片选择与拖拽上传，在「新建图纸」弹窗里取景、选色板与规格并实时预览后生成首版；目标宽度与颜色数量、抖动、主色/平均色取样、亮度/对比度、背景去除与手动背景取样；生成任务可取消 |
| 像素编辑 | 画笔、橡皮、吸管、油漆桶、按色号替换、撤销/重做；支持空白画布起稿，也可对已有图纸换色板重映射，保留手工修补 |
| 手机编辑 | 同一个编辑器的手机布局：底部工具栏与底部面板，平移缩放、双指捏合；显式区分手形导航与编辑工具，画笔/橡皮默认精准落点模式 |
| 色板与板型 | 13 套内置色板、自定义色板新建/编辑、套装档位；支持 5mm 与 2.6mm 迷你豆制作规格，板缝与导出几何随规格变化 |
...
| 发现与社区 | 作品搜索、类目与筛选、精选展示、投稿（作者建议标签）及修订审核、引用为私人设计、点赞、评论、举报、作者主页与站内通知 |
...
主导航为「发现 / 创作 / 我的」：桌面使用顶栏，手机使用底部导航。界面基于 Base UI 原语与统一的设计令牌（豆蓝主色、选中态深墨、每视区最多一个主按钮），图纸缩略图与查看器都是「豆粒渲染」。编辑画布保持中性背景，原图参照窗单向跟随画布范围。

| 入口 | 用途 |
|---|---|
| `/` | 发现：类目、搜索、筛选排序与作品网格 |
| `/app` | 创作入口（选图、空白画布、导入项目、最近设计）与编辑器（编辑、跟拼、分享、导出、公开到豆社） |
| `/community/[id]` | 作品详情：图纸查看器、制作卡、讨论、相似作品与作者的更多作品 |
| `/u/[id]` | 作者主页 |
| `/me`、`/me/public`、`/me/likes`、`/me/palettes`、`/me/settings` | 我的设计、我的公开作品、喜欢、我的色板与账号设置 |
| `/community/submit` | 从云端设计投稿或修改后重新投稿 |
| `/palettes` | 内置色板目录与可生成颜色说明 |
| `/s/[token]` | 只读分享页 |
| `/help`、`/about`、`/privacy` | 使用帮助、项目信息、隐私说明与统计偏好 |
| `/admin` | 按审核员 / 管理员权限开放的管理后台 |

旧地址 `/community`、`/designs`、`/community/mine`、`/account`、`/create` 永久重定向到新页面，查询参数保留。

### 编辑器

选择图片后在「新建图纸」弹窗里确认取景（整图、1:1 或按底板）并生成首版。之后可随时「重新裁剪」更新图纸；取消不会修改现有图纸。有手工编辑时，可能覆盖编辑的操作会先要求确认。
...
| 界面 | Tailwind CSS 4（单一构建、设计令牌）、Base UI 原语 + shadcn 风格组件、lucide-react、react-day-picker、同源交付的中文字体 |
...
```text
src/
  app/                Next.js 页面与 API：发现、创作与编辑器、我的、作品详情、管理后台；globals.css + theme.css 为唯一样式入口
  components/         UI：ui/（Base UI 组件库）、shell/（站点外壳）、create/、editor-workspace/、works/、me/、admin-ui/ 等
