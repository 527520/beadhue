...
import { createBeadhueApi } from './api';
...
describe('createBeadhueApi 云端设计接口', () => {
  it('listDesigns 成功返回列表', async () => {
    const calls: Array<{ url: string; init?: RequestInit }> = [];
    const api = createBeadhueApi(async (input, init) => {
  ...
  it('getDesign 成功返回完整设计', async () => {
    const api = createBeadhueApi(async () =>
  ...
  it('deleteDesign 404 静默成功（幂等），5xx 抛错', async () => {
    const ok = createBeadhueApi(async () => new Response(null, { status: 204 }));
    await expect(ok.deleteDesign('d1', 1)).resolves.toEqual({ updatedAt: '', revision: 2 });

    const notFound = createBeadhueApi(async () => new Response(null, { status: 404 }));
    await expect(notFound.deleteDesign('d1', 1)).resolves.toEqual({ updatedAt: '', revision: 2 });

    const boom = createBeadhueApi(async () => jsonResponse(500, { error: { code: 'INTERNAL' } }));
    await expect(boom.deleteDesign('d1', 1)).rejects.toBeInstanceOf(ApiError);
  });
});

describe('createBeadhueApi 账号接口', () => {
  it('me：401→guest、403→unverified、200→verified', async () => {
    const guest = createBeadhueApi(async () => jsonResponse(401, {}));
    expect(await guest.me()).toEqual({ state: 'guest' });

    const unverified = createBeadhueApi(async () => jsonResponse(403, {}));
    expect(await unverified.me()).toEqual({ state: 'unverified' });

    const verified = createBeadhueApi(async () =>
  ...
  it('非 JSON 错误体：使用默认 code/message', async () => {
    const api = createBeadhueApi(async () => new Response('oops', { status: 502 }));
  ...
  it('resendVerification 成功静默，失败抛错', async () => {
    ...
    const tooMany = createBeadhueApi(async () =>
  ...
  it('changePassword/updateProfile/deleteAccount/logout：成功静默、失败抛错、带 JSON 头', async () => {
    ...
    const fail = createBeadhueApi(async () =>
