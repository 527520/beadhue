...
/**
 * 修订不可变（ADR-0015），同一修订的缩略图只随渲染版本改变：
 ...
 * 地址里的 `v` 只用于让浏览器缓存随渲染版本失效，服务端总是按当前版本渲染。
 */
...
async function get(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const cacheKey = `${revisionId}:${size}:v${THUMBNAIL_RENDER_VERSION}`;
  ...
  const png = renderPatternThumbnail(revision.pattern, { size });
