...
- [ ] （镜像加固，本轮新增）备份改用独立子账号密钥：到 CAM 新建一把只有该 COS 桶 `doupu-backup/` 前缀读写权限的子账号密钥，填 `.env` 的 `BACKUP_COS_SECRET_ID` / `BACKUP_COS_SECRET_KEY`。`COS_SECRET_ID/KEY` 仍留给应用（作品原图）。这一项**必须做**：备份容器已不再回退到应用密钥，未配置时 `backup` 容器启动即失败并告警（`docker compose -f docker-compose.prod.yml logs backup` 会看到 `COS_SECRET_ID/COS_SECRET_KEY/COS_BUCKET/COS_REGION must all be configured`）。
