...
- Sessions: opaque random 32-byte token, **SHA-256 hashed** at rest in the `sessions` table; cookie `beadhue_session`, `HttpOnly`, `SameSite=Lax`, `Secure`, 30-day rolling expiry; logout deletes the row.
