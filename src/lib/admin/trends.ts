...
import { commentModerationChecks, communityComments, communityLikes, communityReports, communityRevisions, users } from '@/../db/schema';
...
export async function getAdminTrends(db: AnyDatabase, input: unknown = {}, now: Date = new Date()) {
  ...
  const [submissions, likes, newUsers, comments, reports, moderationCalls] = await Promise.all([
    ...
    db.select({ day: dayOf(communityComments.createdAt), count }).from(communityComments)
      .where(gte(communityComments.createdAt, start)).groupBy(dayOf(communityComments.createdAt)).then(toMap),
    db.select({ day: dayOf(communityReports.createdAt), count }).from(communityReports)
      .where(gte(communityReports.createdAt, start)).groupBy(dayOf(communityReports.createdAt)).then(toMap),
    db.select({ day: dayOf(commentModerationChecks.createdAt), count }).from(commentModerationChecks)
      .where(and(gte(commentModerationChecks.createdAt, start), eq(commentModerationChecks.provider, 'tencent-tms')))
      .groupBy(dayOf(commentModerationChecks.createdAt)).then(toMap),
  ]);
  const items: TrendDay[] = dates.map((date) => ({
    ...
    comments: comments.get(date) ?? 0, reports: reports.get(date) ?? 0, moderationCalls: moderationCalls.get(date) ?? 0,
  }));
