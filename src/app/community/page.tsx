  ...
  return (
    <main id="main" className="workspace-page">
      ...
      <div className="workspace-content community-page">
        ...
        {shortcutTags.length > 0 && <nav className="community-tag-bar" aria-label={t.tagBar}>
          {shortcutTags.map((tag) => <Link key={tag.id} href={communityTagHref(tag.name)} className="community-tag-chip">{tag.name}<small>{tag.count}</small></Link>)}
        </nav>}
