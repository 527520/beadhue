...
export default function HomeCommunityShelf() {
  ...
  return (
    <section className="home-community">
      <header><div><span className="studio-eyebrow">{state?.featured.length ? t.featured : t.latest}</span><h2>{state?.featured.length ? t.featuredProofs : t.latestProofs}</h2></div><ButtonLink variant="secondary" size="sm" icon="arrow" iconPosition="end" href="/">{t.open}</ButtonLink></header>
